...
import { getSwingManifestFile, SWING_FILE, URI_PATTERN } from "../../constants";
...
async function addDependencyLink(
  ...
  const files = (await vscode.workspace.fs.readDirectory(store.activeSwing!.rootUri)).map(
    ([file]) => file
  );
  const manifestFile = getSwingManifestFile(files) || SWING_FILE;
  const uri = Uri.joinPath(store.activeSwing!.rootUri, manifestFile);
