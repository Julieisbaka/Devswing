...
DevSwing is an interactive coding playground for VS Code, that allows you to build/explore/learn rich web applications ("swings"). It's like having the magic of a traditional web playground (e.g. CodePen, JSFiddle), but available directly from your highly-personalized editor: themes, keybindings, and extensions...oh my! When you create swings, you can use your favorite web languages (HTML/Pug, CSS/SCSS/Less, JS/TS) and libraries (React, Vue, Angular, etc.) and have a live preview <ins>as you type</ins>, without needing to worry about compiling or bundling anything. It's like a visual REPL for managing runnable code snippets.

<img width="800px" src="https://user-images.githubusercontent.com/116461/103024429-ae37a480-4504-11eb-85ea-37ba9b9a4d9a.gif" />

DevSwing supports [template galleries](#template-galleries), which allows you to create re-usable swing templates. When you're happy with a swing, you can also [export it to CodePen](#codepen) in order to share it with the world! Finally, since DevSwing is a VS Code extension, it can integrate with other VS Code extensions, to enable compelling scenarios such as [managing your swings as GitHub Gists](#gistpad), [recording guided walkthroughs of your swings](#codetour), and [collaborating on a swing in real-time](#live-share). If that doesn't sound fun, then I don't know what is!
...
1. Install the DevSwing extension and then reload VS Code

1. Run the `DevSwing: New Swing...` command, and select the desired template

1. Edit any of the opened files and see the `DevSwing` preview update automatically as you type 🚀

1. _(Optional)_ If you'd like to save this sample to a permanent location, run the `DevSwing: Save Current Swing As...` command and select a directory
...
Furthermore, if you'd like to try out a fun sample, clone the [Rock Paper Scissors](https://github.com/codespaces-contrib/rock-paper-scissors) repo, and open it up in VS Code. DevSwing will detect that it's a swing and automatically layout your editor.

## Creating Swings

After you install the DevSwing extension, you can create new swings at any time, using the following commands:

- `DevSwing: New Swing...` - Creates a temporary swing that allows you to quickly try something out (like a visual REPL!). These swings are stored in your system temp directory in a folder called `devswing`, and are named based on the current date/time.

- `DevSwing: New Swing from Last Template` - Creates a swing using the last template that you used. This can be useful when you tend to use the same template over and over again.

- `DevSwing: New Swing in Directory...` - Creates a swing in a local directory, which allows you to re-open it later, and easily share it with others (e.g. store it in a GitHub repo).

After creating a swing, your editor layout will be automatically setup to accomodate the needs of the selected template. Furthermore, if you open a directory that represents a swing, or you run the `DevSwing: Open Swing...` command, then the selected swing will be automatically launched.

### Language Support

DevSwing comes with support for all major web languages, and allows you to create new swings without worrying about building or bundling anything. When you create a swing, the templates list provides options for getting started quickly
...
Additionally, if your script file ends in `.jsx` or `.tsx`, you can use JSX syntax. DevSwing also recognizes a script file that ends in `.babel` and treats it like TypeScript. Finally, if your script file ends in ".mjs", then it will be treated as a [JavaScript module](#modules).

### Components

In addition to the aforementioned languages, DevSwing also allows you to create component-based swings, using either React, React Native, Vue (single-file components) or Svelte. A component-based swing is unique in that it's a single file that lets you focus on just your component, and have all initialization and rendering logic handled automatically.
...
By default, when you create a swing, it will open in a "Split Left" layout, which vertically stacks the code editors on the left, and allows the preview window to fully occupy the IDE height on the right. However, if you want to change the layout, you can run the `DevSwing: Change Layout` command and select `Grid`, `Preview`, `Split Bottom`, `Split Left Tabbed`, `Split Right`, `Split Right Tabbed` or `Split Top`.

![Layout](https://user-images.githubusercontent.com/116461/71560396-5152fc80-2a1e-11ea-9cff-a9590e1ea779.gif)

Additionally, if you create a swing, that looks best in a specific layout, you can set the `layout` property in the swing's `devswing.json` file (legacy `codeswing.json` is still supported) to either: `grid`, `preview`, `splitBottom`, `splitLeft`, `splitLeftTabbed`, `splitRight`, `splitRightTabbed` or `splitTop`. Then, when you or someone else opens this swing, it will be opened with the specified layout, as opposed to the user's configured default layout.

### Console Output

The swing also provides an output window in order to view any logs written via the `console.log` API. You can bring up the output window, as well as run the swing or open the Chrome Developer Tools, by clicking the respective command icon in the editor toolbar. If you'd like to always open the console when creating/opening swings, you can set the `DevSwing: Show Console` setting to `true`.

<img width="503" src="https://user-images.githubusercontent.com/116461/71384593-0a38b780-2597-11ea-8bba-73f784f6ec76.png" />

Additionally, if you create a swing that depends on the console, you can set the `showConsole` property in the swing's `devswing.json` file (legacy `codeswing.json` is still supported) to `true`. Then, when you or someone else opens this swing, the console will be automatically opened, regardless whether the end-user has configured it to show by default.

### External Libraries

If you need to add any external JavaScript libraries (e.g. `react`) or stylesheets (e.g. `font-awesome`) to your swing, simply click the `Add swing Library` command in the swing "action bar" (or run `DevSwing: Add Library` from the command palette). This will allow you to search for a library or paste a custom library URL. When you select a library, it will be automatically added to your swing.

![Add Library](https://user-images.githubusercontent.com/116461/71629251-4ed4dc00-2bb1-11ea-9488-78c3d71dbacd.gif)

Behind the scenes, this command updates the swing's manifest file (`devswing.json`, with legacy `codeswing.json` support), which you can also open/edit yourself manually if you'd prefer. For more details about the `Script module` option, see [JavaScript modules](#javascript-modules).
...
- **Open Console** - Opens the `DevSwing` console, which allows you to view the output of any `console.log` calls that your swing makes ([details](#console-output)).
...
If you open a directory in VS Code, and that directory represents a swing, then the swing environment will be automatically launched. If you accidentally close the DevSwing preview, then you can re-open it by either reloading VS Code, or running the "DevSwing: Re-Open Workspace Swing" command.

### DevSwing tree view

When you create or open a swing, a new tree view called "DevSwing" will appear on the "Explorer" tab (though you can move it to anywhere you want!). It will display the list of files in the active swing, and allow you to edit them.

If you open a swing directory as a workspace, then the "DevSwing" tree won't appear, since it would be duplicative of the "Explorer" tree itself. Therefore, the "DevSwing" tree is meant to accommodate temporary swings, swings you open from directories other than the active VS Code workspace, or swings you load from [gists](#gistpad).
...
Your swing can customize how the readme is rendered by setting the `readmeBehavior` property in your `devswing.json` file (legacy `codeswing.json` is still supported) to either `previewFooter` (which renders the content beneath the preview content), `inputComment` (which renders the content beneath an [input file](#swing-input)), or `none` (which doesn't render the contents at all).
...
1. Set the `scriptType` property to `module` in the swing's `devswing.json` file (legacy `codeswing.json` is still supported)

Once you're using modules, we'd recommend you use [Skypack](https://skypack.dev) to load your desired libraries. To make this simpler, you can run the `DevSwing: Add Library` command, select `Script module` and then specify the module you'd like to add.

<img width="500px" src="https://user-images.githubusercontent.com/116461/103246626-09a8ce80-4919-11eb-9b01-b3a7a02a5634.gif" />

Additionally, you can import CSS and JSON files into a module. When you import a CSS file, DevSwing will inject the contents of that file into the DOM. When you import a JSON file, the default `export` of the module will be the parsed version of the JSON file.
...
DevSwing includes integration with [CodeTour](https://aka.ms/codetour), which allows you to author interactive walkthroughs of a codebase. This can be helpful for annotating relevant "markers" within a swing, and encouraging users/yourself where to focus. In order to create a code tour for a swing, simply open the swing, and click the `...` menu in the editor window of any of the swing's document or the preview window. Select `Record CodeTour` and then start adding step annotations to your swing's code. Then, anytime you or someone else opens your swing, the tour will be automatically started.
...
1. Specifying a tutorial title, by setting the `tutorial` property in the swing's `devswing.json` file (legacy `codeswing.json` is still supported)
...
If your swing needs to accept user input (e.g. to allow a user to take a challenge, play a game), then you can set the `input` property of your `devswing.json` file (legacy `codeswing.json` is still supported) to an object that includes two properties:
...
> Note: If you want to provide additional help information for your input, you can create a `README.md` file in your swing/tutorial and set the `readmeBehavior` property in your `devswing.json` manifest (legacy `codeswing.json` is still supported) to `inputComment`. This will render the contents of the readme as an inline code comment, directly beneath the input file.
...
Whenever you create a swing, it includes a `devswing.json` file (legacy `codeswing.json` is still supported), which defines the metadata for the swing, including it's behavior, requirements and intended presentation.
...
When you install DevSwing, the following commands are available from the command palette:

- `DevSwing: Initialize Workspace as Swing` - Creates a new swing and stores its files in the root of the active workspace.

- `DevSwing: New Swing...` - Creates a new temporary code swing, which can be useful for doing quick/ad-hoc explorations.

- `DevSwing: New Swing in Directory...` - Creates a new code swing, whose files are saved to a specified directory.

- `DevSwing: New Swing from Last Template` - Creates a new temporary swing using the most recently used template.

- `DevSwing: Open Swing...` - Opens a swing, based on the contents of a specified directory.

- `DevSwing: Open Swing in New Windows...` - Opens a swing in a new VS Code window.

- `DevSwing: Generate Swing with Copilot` - Generates a swing from a Copilot prompt.

Additionally, when you have a swing currently open, the following commands are available:

- `DevSwing: Add Library` - Allows you to insert a new JavaScript (e.g. React) or CSS (e.g. Tailwind) library into your swing.

- `DevSwing: Change Layout` - Allows you to select a pre-defined layout for the active swing.

- `DevSwing: Export to CodePen` - Exports the active swing to a new CodePen.

- `DevSwing: Open Console` - Opens the `DevSwing` output window, which lets you view calls to `console.log` in your swing script.

- `DevSwing: Open Dev Tools` - Launches the Chrome DevTools, connected to the swing preview. This is useful for diagnosing the DOM/JavaScript errors of your swing.

- `DevSwing: Run Swing` - Re-executes the code of the active swing.

## Configuration Settings

- `Devswing: Auto Run` - Specifies when to automatically run the code for a swing. Can be set to one of the following values:

  - `onEdit` _(default)_: Will re-run the swing automatically as you type.
  - `onSave`: Will re-run the swing when you save a file.
  - `never`: Don't automatically re-run the swing, and instead, only run it when the `DevSwing: Run Swing` command is executed.

- `Devswing: Auto Save` - Specifies whether to automatically save your swing files (every 30s). If you've already set the `Files: Auto Save` setting to `afterDelay`, then that setting will be respected. Defaults to `false`.

- `Devswing: Clear Console on Run` - Specifies whether to automatically clear the console when the active swing is run.

- `Devswing: Layout` - Specifies how to layout the editor windows when opening a swing. Can be set to one of the following values:
...
- `Devswing: Launch Behavior` - Specifies how DevSwing should behave when you open a swing workspace. Can be set to one of the following values:
...
- `Devswing: Readme Behavior` - Specifies how to display the contents of a swing's readme, if it has one. Can be set to one of the following values:
...
- `Devswing: Root Directory` - Specifies the directory to create swings in within the open workspace. Defaults to `null`.

- `Devswing: Show Console` - Specifies whether to always show the console when opening a swing. Defaults to `false`.

- `Devswing: Template Galleries` - Specifies the list of template galleries to use, when displaying the available templates when creating a new swing. Defaults to `["web:basic", "web:languages"]`.

- `Devswing: Theme Preview` - Specifies whether to apply Visual Studio Code theme to the preview window. Defaults to `false`.

- `Devswing` integrates with GitHub Copilot chat models for AI generation and refinement.
...
In order to allow other extensions to extend the default behavior and/or settings of DevSwing, the following extensibility points are provided:

### Template Galleries

By default, DevSwing ships with a set of template galleries, however, extensions can contribute new ones by adding a `devswing.templateGalleries` contribution to their `package.json` file. This contribution is simply an array of objects with the following properties:
...
- `url` - A URL that points at the actual template gallery. This URL must refer to a JSON file that conforms to the DevSwing template gallery schema.

### Custom Languages

By default, DevSwing ships with support for a handful of [languages](#additional-language-support) you can use in. However, extensions can introduce support for new languages by adding a `devswing.languages` contribution to their `package.json` file. This contribution point is simply an array of objects with the following properties:
...
When your extension contributes a custom language, DevSwing expects that your extension returns an API, that includes the following members:
