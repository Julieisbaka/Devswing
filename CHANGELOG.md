...
- Added support for reflecting changes to `window.title` in the title of the DevSwing preview tab
- Updated the default AI model to `gpt-4o` (which is much cheaper than Turbo)
- Removed the logic that auto-adds `react` and `react-dom` to the `scripts` array in your `devswing.json` (legacy `codeswing.json`) file.
...
- Introduced a new `DevSwing: Launch Behavior` setting, that allows customizing how DevSwing should behave when you open a swing workspace.
- Introduced a new `DevSwing: Root Directory` setting, that allows specifying the workspace directory that new swings should be created in
- Introduced a new `DevSwing: Initialize Workspace as Swing` command, that allows you to open a folder and turn it into a swing
...
- Introduced the new `DevSwing: Theme Preview` setting, which allows you to theme the swing preview window, to match your VS Code color theme
...
- Introduced a new `DevSwing: New Swing from Last Template` command, that creates a swing from your last used template
- Renamed the `DevSwing: New Scratch Swing...` command to `DevSwing: New Swing...`, and `DevSwing: New Swing..` to `DevSwing: New Swing in Directory...`
- Introduced a new `DevSwing: Save Current Swing As...` command, that lets you save the current swing in a specific location
- Added the `DevSwing: Open Swing in New Window...` command
...
- You can now add/rename/delete files from the DevSwing tree (including files within sub-directories)
...
- Introduced the `DevSwing: Clear Console on Run` setting (defaults to `true`)
...
- Added support for exporting swings to CodePen via the new `DevSwing: Export to CodePen` command
...
- Added the `DevSwing: Open Workspace Swing` command, for re-opening the current workspace's swing after closing it.

# v0.0.3 (12/18/2020)

- Changed the default value of the `DevSwing: Readme Behavior` setting to `none`
